...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if index < self.chunks.len() {
            ...
            bail!("Chunk with type {} not found.", chunk_type)
        }
    }
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        output.extend(Self::STANDARD_HEADER.iter().map(|b| format!("{:02X}", b)));
        output.extend(self.chunks.iter().map(|chunk| chunk.to_string()));
        f.write_str(&output)
    }
}
